...
function generateResponse(input) {
  ...
  if (DEMO_RESPONSES[type]) {
    ...
    if (type === 'smartQA') {
      ...
      resp = resp.replace('{simple_answer}', `这是对"${topic}"的简要回答。该概念是人工智能导论课程中的重要知识点，理解它对后续学习非常关键。`);
      resp = resp.replace('{detailed_explanation}', `让我们深入理解"${topic}"。首先回顾一下相关的前置知识，然后从定义出发，逐步推导其核心原理。在这个过程中，我们需要特别注意以下几点...`);
    }
    if (type === 'reviewPlan') {
      resp = resp.replace('{课程名称}', '《人工智能导论》');
    }
    return resp;
  }
